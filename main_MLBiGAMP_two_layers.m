...
%     Hr=Hr.*Hr_loc;
   ...
   %% Receiver
    ...
    [q_hat,b_hat,x_hat,iter_flag]=ML_BiGAMP(Rec_Y,Y_noise,Hr,Hb,X,var_channel,Hr_loc,Hb_loc,S_loc,HbX_loc,K_p,T_p,N_p,iter,damping,damping_r,Rec_Y_pilot);
%     [q_hat,b_hat,x_hat]=ML_BiGAMP_only(Rec_Y,Y_noise,Hr,Hb,X,var_channel,Hr_loc,Hb_loc,S_loc,HbX_loc,K_p,T_p,N_p,iter,damping,damping_r,Rec_Y_pilot);
    %%=================================================
%%
% Simulation result
...
title(['BiGAMP, M=',num2str(M),',K=',num2str(K),',T=',num2str(T), ',N=',num2str(N),',\beta=',num2str(damping_r),',Tp=',num2str(T_p),',Kp=',num2str(K_p),',Np=',num2str(N_p)]);
xlabel('SNR (dB)');
ylabel('NMSE');
grid on
