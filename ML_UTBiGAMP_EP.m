function [q_hat,b_hat,x_hat,iter_flag]=ML_UTBiGAMP_EP(Rec_Y,Y_noise,Hr,Hb,X,var_channel,Hr_loc,Hb_loc,S_loc,HbX_loc,K_p,T_p,N_p,iter,damping,damping_r,Rec_Y_pilot)
% This function is the part of two-layers BiGAMP algorithm adopted Unitary
[K,T]=size(Rec_Y);
[N,M]=size(Hb);
    ...
    x_dec=zeros(M,T);
        ...
        if any(tt==[1:2])
           ...
           [Z_nt,V_nt,V_bar_nt,b_hat,x_hat,v_b,v_x]=Forw_iter_EP(1,prior_X_mean,prior_X_var,prior_H,Sigma_x,R_x,Sigma_b,R_b,s_tilde_nt,X,T_p,Hr(1:K_p,:),K_p,V_nt_former,b_hat_former,N_p,Hb,Z_nt_pilot,V_nt_pilot,damping,tt,S_loc,Hb_loc,HbX_loc,Hr_loc(1:K_p,:));
           V_nt_former=V_nt;
           b_hat_former=b_hat;
...
%         A_eq=(Hb*X).';
        ...
        [q_hat_new,v_q_new,u_hat_new,v_u_new]=Single_GAMP(prior_X_mean_new,prior_X_var_new,prior_H_mean_new,prior_H_var_new,A_eq_new,T,damping_r,rec_new,Y_noise.',Hr_loc.',var_channel,iter,Hr.',K_p);
        q_hat=q_hat_new.';
        v_q=v_q_new.';
        u_hat=(U_A*u_hat_new).';
        v_u=v_u_new.';
%         
        for tt=1:4
           ...
           [Z_nt,V_nt,V_bar_nt,b_hat,x_hat,v_b,v_x]=Forw_iter_EP(1,prior_X_mean,prior_X_var,prior_H,Sigma_x,R_x,Sigma_b,R_b,s_tilde_nt,X,T_p,q_hat_new.',K,V_nt_former,b_hat_former,N_p,Hb,Z_nt_pilot,V_nt_pilot,0.7,tt,S_loc,Hb_loc,HbX_loc,Hr_loc);
           V_nt_former=V_nt;
           b_hat_former=b_hat;
           [Z_kt,V_kt,V_bar_kt,q_hat,u_hat,v_q,v_u]=Forw_iter(2,Z_nt,V_nt,prior_Q,Sigma_u,R_u,Sigma_q,R_q,s_tilde_kt,X,T_p,q_hat_new.',K,V_kt_former,q_hat_former,N_p,Hb,Z_nt_pilot,V_nt_pilot,0.7,tt,S_loc,Hb_loc,HbX_loc,Hr_loc); 
           V_kt_former=V_kt; 
           q_hat_former=q_hat;
        end
...
%         A_eq=(Hb*X).';
        ...
        prior_H_mean_new=zeros(T,N);
        prior_H_var_new=ones(T,N);
        [q_hat_new,v_q_new,u_hat_new,v_u_new]=BiGAMP(prior_X_mean_new,prior_X_var_new,prior_H_mean_new,prior_H_var_new,A_eq_new,T,damping_r,rec_new,Y_noise.',Hr_loc.',var_channel,iter,Hr.',K_p);
        q_hat=q_hat_new.';
        v_q=v_q_new.';
        u_hat=(U_A*u_hat_new).';
        v_u=v_u_new.';
...
%          if numel(find(isnan(q_hat)))
%              q_hat=Hr;
%          end
%          if numel(find(isnan(X)))
%              x_hat=X;
%          end
%          if numel(find(isnan(b_hat)))
%              b_hat=Hb;
%          end
