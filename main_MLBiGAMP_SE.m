%========================================================a
%  Author: Wei Li;
%  Date:   2021.Feb.22,  SUTD;
%  Version: V1.0 
%  Note: The State Evolutional Code. This is the code for MP algorithm in 
%     RIS-assisted system that estimates transmitted signal and two 
%     channels without PARAFAC structure. 
...
T_p=160;% The number of pilot symbols
N=200; % The number of elements placed on RIS
K_p=300; % The number of pilot in channel H^r
...
%     Hr_loc=ones(K,N);
   ...
   %% Receiver
    ...
    [q_hat,b_hat,x_hat,se_x,se_b,se_r]=ML_BiGAMP_SE(Rec_Y,Y_noise,Hr,Hb,X,var_channel,Hr_loc,Hb_loc,S_loc,HbX_loc,K_p,T_p,N_p,iter,damping,damping_r,Rec_Y_pilot);
     %%=================================================
%%
% Simulation result
...
%       MSE_part2=norm(Hb*X -A_eq_hat_new.','fro')^2/(norm(Hb*x_hat,'fro')^2);
      ...
      se_x_sum=se_x_sum+se_x;
      se_b_sum=se_b_sum+se_b;
      se_r_sum=se_r_sum+se_r;
%%========================================================%
    end
    ...
    se_x_cur(1,snr)=se_x_sum/FRAME;
    se_b_cur(1,snr)=se_b_sum/FRAME;
    se_r_cur(1,snr)=se_r_sum/FRAME;
...
hold on
semilogy(0:5:30,se_x_cur,'rx','linewidth',2)
hold on
semilogy(0:5:30,se_b_cur,'mx','linewidth',2)
hold on
semilogy(0:5:30,se_r_cur,'bx','linewidth',2)
hold on
legend({'Proposed MP (X)','Proposed MP (H^b)','Proposed MP (H^r)','Proposed MP (all)','SE (X)','SE (H^b)','SE (H^r)'});
