...
T=100; % The numebr of time slots
T_p=100;% The number of pilot symbols
T_d=100; % The number of data symbols
...
%     Hr=Hr.*Hr_loc;
    ...
    X_data=sqrt(var_channel)*(randn(M,T_d) );
    rec_x_pilot_true=Hr*Hb*X_data+sqrt(var_noise)*(randn(K,T_d));
    TEMP=Rec_Y';
   ...
   %% Receiver
    ...
    [q_hat,b_hat,x_hat]=ML_BiGAMP(Rec_Y,Y_noise,Hr,Hb,X,var_channel,Hr_loc,Hb_loc,S_loc,HbX_loc,K_p,T_p,N_p,iter,damping,damping_r,Rec_Y_pilot);
     %%=================================================
%%  Ls estimation of X in data part
    H_eq_est=q_hat*b_hat;
    x_data_est=pinv(H_eq_est)*rec_x_pilot_true;

% Simulation result
      MSE_X=norm(X_data- x_data_est,'fro')^2/(norm(X_data,'fro')^2);
...
semilogy(0:5:30,diff_X,'-x','Color',[0.85,0.33,0.1],'linewidth',2)
hold on
semilogy(0:5:30,diff_Hb,'-<','Color',[0.72,0.27,1],'linewidth',2)
hold on
semilogy(0:5:30,diff_Hr,'->','Color',[0,0.45,0.74],'linewidth',2)
hold on
...
title(['BiGAMP+LS, M=',num2str(M),',K=',num2str(K),',T=',num2str(T), ',N=',num2str(N),',\beta=',num2str(damping_r),',Tp=',num2str(T_p),',Kp=',num2str(K_p)]);
xlabel('SNR (dB)');
ylabel('NMSE');
grid on
